...
String.prototype.convertToSlug = function() {
    ...
    for(var i = 0; i<this.length; i++){
        str += map[ this.charCodeAt(i).toString(16) ] || "";
    }
...
$(function(){
    ...
    var $contacts = $('#contacts li:not(.sep) a');
    $contacts.sort(function() { return 0.5 - Math.random() });
    
    var $conversations = $('#inbox > div');

    $contacts.each(function(){
        var contact_html = $(this).html();
        var $contactname = $('<h2>' + contact_html + '</h2>')
        var $section= $('<section id="' + $(this).text().convertToSlug() + '"></section>');
        $conversations.append($section);

        $section.append($contactname);
        var random_messages = messages.sort(function() { return 0.5 - Math.random() });

        var nbofmessages = Math.round(Math.random()*8) + 2;

        for (var i = 0; i < nbofmessages; i++) {
            var sendOrReceived = Math.random() > .5 ? 'sent' : 'received';
            var $message = $('<div class="' + sendOrReceived + '">' + random_messages[i] + '</div>');
    ...
    /* ------------------------------------------------------------------------------------------- utils screens */
    $('#saisir, #contacts, #inbox, #thread' ).css({
        'height': $('#home').outerHeight()
    })

    hideScreens = function(){
        $screens = $('.screen');
        $screens.addClass('invisible');
        var s = setTimeout(function(){
            $screens.removeClass('visible');
            $screens.removeClass('invisible');
        }, 400)
    }

    showScreen = function(myscreen){
        myscreen.removeClass('invisible');
        //myscreen.css('left', 380)
        
        var s = setTimeout(function(){
        myscreen.addClass('visible')    
        }, 400)
    }
    ...
    /* ------------------------------------------------------------------------------------------- Conversations */
    $('#inbox section').on('click', function(){
        switchToThread($(this).attr('id'))
    })

    switchToThread = function(contact){
        var $record_btn = $('<div id="thread-button-record-' +  contact + '" class="button-record button visible sent"></div>');

        var $thread = $('#thread');
        $thread.addClass('visible alpha');
        $source = $('#' + contact);
        $thread.find('.nano-content').html($source.html())
        $thread.find('.nano-content').append($record_btn);
        catchRecordButtons();
        hideScreens();
        showScreen($thread);
        $(".nano").nanoScroller();
        ...
        // populate contact
        var $contact_el = $('#contacts').find('[data-id=' + contact + ']') 
        $('#contact a').attr('data-id', contact)
        $('#contact a').html($contact_el.html())


        var s = setTimeout(function(){
            // var h = $thread.height();
            // console.log(h)
            $thread.removeClass('alpha')
    ...
    $('#contacts a').each(function(){
        $(this).attr('data-id', $(this).text().convertToSlug())
    });
    ...
    $('#contacts a').on('click', function(event){
        ...
        $('#contact a').attr('data-id', $(this).attr('data-id'))
    ...
    /* ------------------------------------------------------------------------------------------- Saisir */
    catchRecordButtons = function(){
        $('.button-record').on('click', function(event) {
            ...
            $('#button-record').show();
            $('#button-stop').hide();
            $('.bottomnav').removeClass('below')

            var $this = $(this);
            if($this.attr('id') == 'button-record'){
                $(this).hide();
            } else {
    ...
    $('#button-send').on('click', function(){
        ...
        var $message_container = $('<div class="sent"></div>');
        var message = $('#transcription').html();
        $message_container.html(message);

        var qui = $('#contact a').attr('data-id');
        ...
        showScreen($('#inbox'));
        var $thread = $('#' + qui);
        $thread.append($message_container)
        switchToThread(qui);
    })
